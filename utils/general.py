...
def scale_coords_new(img1_shape, coords, img0_shape, ratio_pad=None):
    ...
    coords[2] /= gain
    coords[3] /= gain
    clip_coords_new(coords, img0_shape)
    # print(coords)
    return [int(i)for i in coords]

def clip_coords_new(boxes, img_shape):
    # Clip bounding xyxy bounding boxes to image shape (height, width)
    if boxes[0]<0:
        boxes[0]=0
    elif boxes[0]>img_shape[1]:
        boxes[0]=img_shape[1]
    if boxes[1]<0:
        boxes[1]=0
    elif boxes[1]>img_shape[0]:
        boxes[1]=img_shape[0]
    if boxes[2]<0:
        boxes[2]=0
    elif boxes[2]>img_shape[1]:
        boxes[2]=img_shape[1]
    if boxes[3]<0:
        boxes[3]=0
    elif boxes[3]>img_shape[0]:
        boxes[3]=img_shape[0]
